...
async def action_get_admin_analysis():
    ...
    active_status = await account.find_many(account.status == True).to_list()
    active_user_num = len(active_status)
    premium_user = await account.find_many(account.payment_info_id != []).to_list()
    premium_user_num = len(premium_user)
